...
const OrderPage = () => {
    ...
    return (
        <div style={{ backgroundColor: "#f5f6fa", width: "100%", paddingLeft: "30px", paddingRight: "30px" }}>
            <Box sx={{ display: "flex", marginLeft: "17px" }}>
                <Typography sx={{ fontSize: "32px", color: "#202224", marginTop: "40px",fontWeight:600 }}>Order Lists</Typography>
            </Box>
            <Box
                display="flex"
                alignItems="center"
            ...
            >
                ...
                <Box display="flex" alignItems="center" height="50px" gap="25px" paddingLeft="13px"> 
                    <Typography sx={{ fontSize: "14px", color: "#202224", fontWeight:600 }}>Filter By</Typography>
                    <Box
                        sx={{
                            borderRight: "1px solid #979797",
                            height: "calc(100% + 13px)"
                ...
                >
                    <Button
                        onClick={handleClick}
                        sx={{ fontSize: "14px", color: "#202224", fontWeight:600 }}
                    >
                        Date
    ...
    renderValue={() => (
        <Box display="flex" alignItems="center" gap="10px" height="30px">
            <Button sx={{ fontSize: "14px", color: "#202224", textTransform: "none", height: "100%",fontWeight:600 }}>
            Order Type
...
>    onChange={(e) => setOrderStatus(e.target.value)}
    ...
    renderValue={() => (
        <Box display="flex" alignItems="center" gap="90px" height="30px">
            <Button sx={{ fontSize: "14px", color: "#202224", fontWeight:600, height: "100%" }}>
            Order Status
                <img src={downarrow} alt="down arrow" style={{ marginLeft:"30px" }} />
            </Button>
            <Box
...
>    
    ...
    <MenuItem value="store">In-Store</MenuItem>
                ...
                <Button
                    ...
                    color="#EA0234"
                    onClick={handleReset}
                    sx={{ textTransform: "none",color:"#EA0234"}}
                >
                    Reset Filter
                </Button>
            </Box>
            <Box sx={{ padding: "20px",backgroundColor:"white",borderRadius:"14px",border:"1px solid #D5D5D5",ml:"20px",mt:"18px" }}>
            <TableContainer>
                <Table>
                    {/* Table Head */}
                    <TableHead>
                    ...
                    <TableBody>
                        {orders.map((order,index) => (
                            <TableRow key={order.id}>
                                <TableCell sx={{fontSize:"14px",fontWeight:600,borderBottom: index === orders.length - 1 ? "none" : "1px solid rgba(224, 224, 224, 1)",}} style={{color:"rgba(32, 34, 36, 0.9"}}>{order.id}</TableCell>
        ...
        borderBottom: index === orders.length - 1 ? "none" : "1px solid rgba(224, 224, 224, 1)",
                                    ...
                                    <Chip
                                        label={order.status}
                                        sx={{ fontWeight: "bold" ,borderBottom: index === orders.length - 1 ? "none" : "1px solid rgba(224, 224, 224, 1)",borderRadius:"4.5px", bgcolor: statusColors[order.status] || "default",color: statusTextColors[order.status] || "default" }}
                                    />
                                </TableCell>
                            </TableRow>
                        ))}
